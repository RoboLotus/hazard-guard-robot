...
재활용 처리장을 **8 m × 6 m** 방에 담은 데모 공간이며, 설비 메시는 원본을
`0.0983`배 한 것입니다. `hall_shell`을 제외한 **설비 7종이 모두 들어갑니다.**

원본 레이아웃을 그대로 균등 축소하면 쓸 수 없습니다. 처리장은 벙커부터 베일
창고까지 설비가 벽에서 벽까지 이어져 있어 어느 배율에서도 순환로가 생기지 않고
왕복 통로만 남습니다. 그래서 데모 월드는 설비를 **남측 벽 / 중앙 섬 / 북측 벽
3단**으로 재배치해 섬을 도는 순찰 경로를 만듭니다.

축척 `0.0983`은 고정값이 아니라 탐색 결과입니다. 방 크기가 입력이고,
생성기가 순환로 병목 폭이 목표치 1.20 m에 도달할 때까지 축척을 낮춥니다.
현재 병목 폭은 1.23 m로 M1의 최소 요구치(풋프린트 0.31 m + 인플레이션
0.35 m = 1.01 m)를 넘습니다.

레이아웃은 `tools/gen_demo_world.py`가 생성하고 동시에 검증합니다. 방 크기를
바꾸거나 설비를 넣고 빼려면 그 스크립트의 `HALL_X`/`HALL_Y`와 밴드 목록
(`ISLAND`, `SOUTH`, `NORTH`)을 수정한 뒤 다시 실행하십시오. 월드 SDF는
생성물이므로 직접 편집하지 마십시오.

### 데모 설비 배치 (실물 제작 치수)

| 설비 | 밴드 | 회전 | 가로 | 세로 | 높이 | 중심 X | 중심 Y |
|---|---|---|---|---|---|---|---|
| sorting_line | 중앙 | 0° | 1.86 m | 1.28 m | 0.49 m | −0.91 | −0.04 |
| baler | 중앙 | 90° | 1.82 m | 0.92 m | 0.53 m | 0.93 | −0.04 |
| control_room | 남측 | 0° | 1.06 m | 0.65 m | 0.36 m | −1.53 | −2.68 |
| primary_shredder | 남측 | 90° | 1.37 m | 1.09 m | 0.67 m | −0.32 | −2.45 |
| bale_storage | 남측 | 90° | 1.69 m | 0.53 m | 0.45 m | 1.22 | −2.73 |
| bunker | 북측 | 90° | 2.63 m | 1.16 m | 0.71 m | −0.70 | 2.42 |
| secondary_processor | 북측 | 90° | 1.40 m | 1.04 m | 0.69 m | 1.31 | 2.48 |
...
| 지점 | X | Y | 통로 여유 |
|---|---|---|---|
| 북 (벙커 앞) | 0.03 | 1.37 | 1.01 m |
| 동 (압축기 옆) | 2.91 | −0.03 | 2.16 m |
| 남 (파쇄기 앞) | −0.01 | −1.51 | 1.04 m |
| 서 (관리실 앞) | −2.91 | −0.03 | 2.16 m |
