...
**배치는 `RoboLotus/gazebo-simulator` 원본 그대로입니다.** 같은 모델 8종을 같은
include 순서로, 전부 공통 월드 원점에 회전 없이 배치합니다. 원본과 다른 것은
균등 축척 `0.13201` 하나뿐이며, 이는 60.60 m × 35.60 m 시설을 8 m × 6 m 방에
넣을 수 있는 최대값입니다. 통로 폭이 축척에 비례하므로 클수록 유리합니다.
...
| 항목 | 값 |
|---|---|
| 주행 가능 면적 | 4.74 m² (방의 10%) |
| 주행 범위 | x −1.81 … 2.11 m, y −1.79 … −0.11 m |
| 최대 통로 폭 | 2.64 m |
...
| bale_storage | 0.71 m | 2.27 m | 0.61 m | 3.40 | 0.63 |
| control_room | 1.43 m | 0.87 m | 0.48 m | 3.14 | −1.80 |
