...
from hazard_guard_thermal_analysis.projection import (
    CameraIntrinsics,
    RigidTransform,
...
def test_identity_projection_attaches_temperature() -> None:
    camera = CameraIntrinsics(3, 3, 1.0, 1.0, 1.0, 1.0)
    depth = [1.0] * 9
    temperatures = [20.0 + index for index in range(9)]
...
def test_rigid_transform_rotates_and_translates() -> None:
    ...
    assert transform.apply(1.0, 0.0, 0.0) == pytest.approx(
        (1.0, 1.0, 0.0)
    )
