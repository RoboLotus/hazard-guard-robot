...
def make_manager(current_pose):
    ...
    parameter_values = {
        ...
        "pre_rotation_yaw_tolerance_rad": 0.10,
    }
    ...
    manager._spin_to_heading_with_safety_retry = (
        lambda yaw, frame_id, goal: manager.spin_calls.append(
            (yaw, frame_id, goal)
        )
    )
...
def test_waypoint_navigation_uses_forward_transit_then_inspection_heading():
    ...
    assert len(manager.spin_calls) == 1
    assert math.isclose(manager.spin_calls[0][0], math.pi / 4.0)
...
def test_short_waypoint_leg_uses_one_final_pose_goal():
    ...
    assert manager.spin_calls == []


def test_already_aligned_robot_skips_pre_rotation():
    manager = make_manager((0.0, 0.0, math.pi / 4.0))
    target = (2.0, 2.0, -math.pi / 2.0)
